...
class MockCharacterTraitsRepository {
  ...
  async findByCharacterId(characterId: number): Promise<CharacterTraits | null> {
    return this.traits;
  }
...
describe('GetFusedDataUseCase Integration', () => {
  ...
  beforeEach(() => {
    ...
    pokemonMatchingService = new PokemonMatchingService(mockPokeApiService);
  ...
  describe('execute', () => {
    it('should successfully execute fusion with cache miss', async () => {
      ...
      mockSwapiService.getCharacter.mockResolvedValue(mockCharacter);
    ...
    it('should handle multiple fusion limit', async () => {
      mockCacheService.getFusionResult.mockResolvedValue(null);
      mockSwapiService.getRandomCharacterId.mockResolvedValueOnce(1).mockResolvedValueOnce(2);
    ...
    it('should use existing character traits when available', async () => {
      ...
      expect(characterTraitsRepository.findByCharacterId).toHaveBeenCalledWith(1);
