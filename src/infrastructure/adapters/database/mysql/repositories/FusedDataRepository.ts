...
export class FusedDataRepository implements IFusedDataRepository {
  ...
  public async findByUserId(
    ...
    const data = (dataRows as any[]).map(row => this.mapRowToFusedCharacter(row));
  ...
  private mapRowToFusedCharacter(row: any): FusedCharacter {
    const fullResponse = JSON.parse(row.full_response);
    ...
    return new FusedCharacter(
      ...
      fusionStrategy,
      {
        ...
        matchingTraits: JSON.parse(row.matching_traits),
